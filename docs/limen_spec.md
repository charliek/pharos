...
3. At an equal key within either tier, a query-**conditioned** route (declaring either field) beats an unconditioned one. This is what lets a narrow exception sit alongside the general route for a path, in either config order. It applies *only* at an equal key: a query condition never re-orders rules 1–2, so — for route pairs validation accepts — a narrower template or a longer prefix still wins over a broader route that carries a condition (the one narrower-over-conditioned pair that would make this a steal is refused at load time; see the overlap table).
4. Config order remains the final stable tiebreak.

Path rank is therefore settled before any query condition is consulted, and that holds whether one, both, or neither route of a pair is conditioned. A conditioned narrower template over a conditioned broader one is ordinary refinement: the narrower wins on the paths it matches, and its condition narrows only its own shape — an explicit declaration by the operator, not an ambiguity. The single subsumption-ordered pair that is refused is the steal orientation the overlap table names: a narrower **unconditioned** template over a broader query-conditioned one, where the narrower would take exactly the requests the broader route's condition was written to capture.

Two conditioned routes that could both match one request **at an equal path rank** — the same `path_prefix`, or two templates of the same shape (parameter names aside), where no specificity is left to order them — are rejected at load time unless provably disjoint, as is any path-template pair or template/prefix pair the overlap table above refuses (Section 5.3). So this ordering never has to choose arbitrarily.
...
A **global** limit on concurrent in-flight shadow requests — one budget for the whole proxy; a per-route budget is not implemented. When exceeded, shadows are **skipped** (not queued unboundedly), incrementing `shadow_skipped{reason="concurrency_limit"}`. Protects the proxy and the new upstream under load. The limit is a single **global** budget, not a per-route one, so this skip's cost is not confined to whichever route momentarily consumed the slots: `limen verdict`'s floors check fails a floored route's evidence outright on a single recorded `concurrency_limit` skip against it, even once that route's own comparison count clears its floor — the route that saturated the cap and the route whose floor it undermines can be two different routes, which is why the remedy is to raise the cap or lower drive concurrency rather than hunt for a culprit route.
...
Its defining property is negative: **it must be unable to render a failure or a missing input as success.** The banner has three states — CLEAN, INCOMPLETE, FAILURE — and reaching CLEAN requires the sink directory, `--config` and `--verdict` all present and parsed, every *provided* optional input parsed, a self-consistent verdict that exited 0 online, and no disagreement between artifacts. Sink counts are reconciled against the verdict's per-route map, canary records against its `canary_records`, verdict floors against the config's `effective_min_comparisons()`, and — since a floored route's evidence is now two claims rather than one — a floors row's `skipped` and `shadow_failures` against the scrape's own per-route sums of the families they were read from; any disagreement is a named finding and a FAILURE. `floor_met` (the arithmetic claim: did the comparison count reach the floor) and `met` (`floor_met` plus no uncompared sampled work) are reported separately, so a route renders as STARVED — below its floor — or UNDERMINED — at its floor but with sampled work that never reached the comparison engine; a row reporting `met: true` alongside any uncompared work named against it is itself an impossible artifact, and a FAILURE rather than a clean row. Where the page reads an input `limen verdict` also reads, it takes the same position on it — including which metric families may legitimately be absent: the pipeline series and the three uncompared families (`limen_shadow_skipped_total`, `limen_comparison_skipped_total`, `limen_shadow_failed_total`) are pre-registered per configured route at zero and required here exactly as they are for the verdict itself; the lazily-registered remainder is not.
...
**Path templates:** matching is segment-count-exact (a shorter or longer path never matches); a parameter never matches an empty segment, and neither `//` nor a trailing slash matches — such a path falls through to the prefix tier instead; matching compares raw bytes and never percent-decodes, so `%2F` stays inside its segment and never becomes a boundary; co-matchability and subsumption (a literal refines a parameter), with a concrete witness path for a co-matchable pair; template-vs-prefix containment predicates (a prefix covering whole segments contains the template; one ending mid-segment, one pinning a parameter position, one longer than the template, or one with an empty segment does not), cross-checked against enumeration. Each row of the overlap decision table: a strictly narrower template is accepted; a co-matchable pair where neither is narrower is refused; identical shapes fall back to the equal-prefix query rules (exactly one conditioned accepted, both conditioned only if provably disjoint, neither conditioned refused); a template contained in a prefix route is accepted and one that half-covers it refused; a template over a query-conditioned prefix needs a complementary condition. Both conditioned orientations of a subsumption-ordered pair are pinned: a narrower **unconditioned** template over a broader conditioned one is refused (it would steal the condition's requests), a narrower **conditioned** one over a broader conditioned one is accepted (ordinary refinement). Precedence: the template tier is consulted ahead of the prefix tier; within it fewer parameters wins; a conditioned template beats an unconditioned one of the same shape; templates of different lengths never compete; and swapping config order does not change the winner. Provenance: a matcher reports its `match_basis` and normalizes the recorded path only for a template route; a profile lacking `match_basis` fails to deserialize; and `suggest-routes` treats a profile recorded under a different matcher than the config now declares as input unavailable, while an agreeing profile — and a route the profile never carried — passes.

**Config validation:** valid minimal config; invalid upstream URL; percentage out of range; missing required route fields; invalid timeouts; out-of-subset JSONPath; duplicate route IDs; unknown route mode; both contract-ref and inline behavioral block on one route fails; `failover_to_legacy` route with non-idempotent methods missing `failover_safe: true` fails; out-of-range `budget` values fail; empty/duplicate query-condition names fail; percent-encoded, `+`-bearing, or whitespace-padded query-condition names fail; a name in both `query_present` and `query_absent` fails; two query-conditioned routes on one prefix fail unless provably disjoint; a `match` setting both or neither of `path_prefix` / `path_template` fails; an unparseable template (no leading `/`, an empty segment, a trailing slash, the bare `/`, a parameter not spanning its segment, a non-identifier or repeated parameter name, no literal segment) fails on the `path_template` field; a strictly narrower path template refines a broader one whether or not either carries a query condition, but a narrower *unconditioned* template over a broader conditioned one fails.
